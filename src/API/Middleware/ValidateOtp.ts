...
import { findOtpBy } from "../Models/OtpModelModel";
...
export async function validateOtp(req: Request, res: Response, next: NextFunction) {
    ...
    const token = targetOtp?.expiredAt.toString();  // Convert Date to string
    if (token) {
        jwt.verify(token, process.env.ACCESS_TOKEN_SECRET_KEY || "hello world", async (error: any) => {
            if (error) {
                return res.status(ResStatus.BAD_REQUEST).json(errorResponseTemplate(
