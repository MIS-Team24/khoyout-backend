...
async function resetPasswordHandler(req, res, next) {
    try {
        ...
        if (!userTarget) {
            return res.status(main_1.ErrorStatus.BAD_REQUEST).json((0, ErrorTemplate_1.errorResponseTemplate)(new badRequest_1.BadRequestException(Messages_1.Messages.USER_NOT_FOUND, main_1.ErrorCode.USER_NOT_FOUND, { isPasswordUpdated: false, success: false })));
        }
        //
        //if password amd repeated password not the same
        if (passwordResetBody.password != passwordResetBody.repeatPassword) {
            return res.status(main_1.ErrorStatus.BAD_REQUEST).json((0, ErrorTemplate_1.errorResponseTemplate)(new badRequest_1.BadRequestException(Messages_1.Messages.PASS_NOT_R_PASS, main_1.ErrorCode.PASSWORD_NOT_REPEATED_PASSWORD, { success: false, isPasswordUpdated: false })));
        }
        //
        //hash password
        ...
        console.log(error);
        return res.status(main_1.ErrorStatus.SERVER_ERROR).json((0, ErrorTemplate_1.errorResponseTemplate)(new badServer_1.BadServerException(Messages_1.Messages.SERVER_ERROR, main_1.ErrorCode.SERVER_ERROR, { success: false, isPasswordUpdated: false, error })));
    }
}
