...
import { randUrl, randNumber, randJobTitle, randFullName, randParagraph, randProductDescription } from '@ngneat/falso';
...
const predefinedDesigners = [
  ...
  },
  {
    ...
    async function main(): Promise<void> {
      for (const designer of predefinedDesigners) {
          shuffleArray(predefinedServices);
          const servicesCount = faker.number.int({ min: 2, max: 4 });
  
          shuffleArray(teamMemberNames);
          const teamMembersCount = faker.number.int({ min: 2, max: 3 });
          const selectedTeamMembers = teamMemberNames.slice(0, teamMembersCount).map(name => ({
              name: name,
              role: faker.helpers.arrayElement(teamMemberRoles),
              avatarUrl: randUrl() + '.jpg'
          }));
  
          const createdDesigner = await prisma.designer.create({
              data: {
                  name: designer.name,
                  ...
                  workingDays: JSON.stringify({
                        Monday: faker.datatype.boolean() ? "9:00 AM - 5:00 PM" : null,
                        Tuesday: faker.datatype.boolean() ? "9:00 AM - 5:00 PM" : null,
                        Wednesday: faker.datatype.boolean() ? "9:00 AM - 5:00 PM" : null,
                        Thursday: faker.datatype.boolean() ? "9:00 AM - 5:00 PM" : null,
                        Friday: faker.datatype.boolean() ? "9:00 AM - 5:00 PM" : null,
                        Saturday: faker.datatype.boolean() ? "9:00 AM - 5:00 PM" : null,
                        Sunday: faker.datatype.boolean() ? "9:00 AM - 5:00 PM" : null,
                  }),
                  avatarUrl: randUrl() + '.jpg',
                  ordersFinished: randNumber({ min: 0, max: 50 }),
                  rating: faker.number.int({ min: 3, max: 5 }),
                  ratingCount: faker.number.int({ min: 1, max: 10 }),
                  services: {
                      create: predefinedServices.slice(0, servicesCount).map(service => ({
                          title: service.title,
                          description: service.description,
                          price: faker.number.int({ min: service.minPrice, max: service.minPrice + 150 })
                      }))
                  },
                  teamMembers: {
                      create: selectedTeamMembers
                  }
              }
          });
  
          const numReviews = faker.number.int({ min: 1, max: 10 });
          console.log(`Designer created: ${createdDesigner.name} with ${servicesCount} services, ${teamMembersCount} team members, ${numReviews} reviews, average rating of ${createdDesigner.rating}, and ${createdDesigner.ratingCount} rating counts.`);
      }
  }
